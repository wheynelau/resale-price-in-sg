...
avg = st.session_state.data.groupby('year').agg({'resale_price':np.median})
sns.lineplot(data=avg,x='year',y='resale_price', ax=axarr[0][0])
axarr[0][0].set_title('Resale Price by Year')

year_subset = st.session_state.data[st.session_state.data['year'] ==
                                     st.session_state.data['year'].max()]
ytd_avg = year_subset.groupby('month').agg({'resale_price':np.median})
sns.lineplot(data=ytd_avg,x='month',y='resale_price', ax=axarr[0][1])
axarr[0][1].set_title('YTD Median Resale Price by Month in {}'.format(st.session_state.data['year'].max()))
# PLOT 3
avg_town_price= st.session_state.data.groupby('town')['resale_price'].median()
avg_town_price = avg_town_price.to_frame()
sns.barplot(data=avg_town_price,
...
sns.lineplot(data=mop_resale,x='year',y='town'
             ,ax=axarr[1][1])
